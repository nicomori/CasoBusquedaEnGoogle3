package zenjob.testautomation.sync.translators
        ...
        currentSections.each { def currentSection ->
            ...
            if (currentSection.testCases.size() > 0) {
                ...
                if (currentSection.testCases && currentSection.description != null) {
                    featureFile.background = new FileSystemSchema.Scenario(null, null, currentSection.description)
                }

                currentSection.testCases.each {
                    FileSystemSchema.Scenario scenario = new FileSystemSchema.Scenario(it.id, it.name, it.content)
