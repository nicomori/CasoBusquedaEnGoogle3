package framework.src.Pages.TVS.old_pepe.mobile

import framework.src.Pages.TVS.old_pepe.mobile.helper.GebMobileScreenshotRule
...
class GebMobileBaseSpec extends GebSpec {
    ...
    public static String CONTEXT_WEBVIEW_PREFIX = "WEBVIEW"

    String findWebViewContext(){
        driver.getContextHandles().grep{ String ctx -> ctx.startsWith(CONTEXT_WEBVIEW_PREFIX)}.first()
    }
    ...
    public def withWebView(Closure closure) {
        withContext(findWebViewContext(), closure)
    }
